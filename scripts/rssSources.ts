export const rssSources = [
  {
    name: "FINAL FANTASY XIV, The Lodestone",
    url: "https://jp.finalfantasyxiv.com/lodestone/news/news.xml",
    category: "FF14",
  },
  {
    name: "ファイナルファンタジーXIV: 公式ブログ",
    url: "https://jp.finalfantasyxiv.com/blog/atom.xml",
    category: "FF14",
  },
  {
    name: "ヤーン速報",
    url: "https://yan-flash.com/feed.xml",
    category: "FF14",
  },
  {
    name: "シャキ待ち速報",
    url: "https://syakimachi.com/rss.xml",
    category: "FF14",
  },
  {
    name: "FF14ひかせん速報",
    url: "https://ff14hikasensokuhou.com/?xml",
    category: "FF14",
  },
  {
    name: "全てのヒカセンたちへ",
    url: "https://ff14isgot.livedoor.blog/index.rdf",
    category: "FF14",
  },
  {
    name: "ドリッピー速報",
    url: "https://drippy-sokuhou.com/feed.xml",
    category: "FF14",
  },
  {
    name: "任天堂ホームページ",
    url: "https://www.nintendo.co.jp/news/whatsnew.xml?_gl=1*13w0cgw*_gcl_au*MTQ1MTQ0NzE3Ny4xNzczMDI3Nzky*_ga*MTAwNzMxMTY2MS4xNzczMDI3Nzky*_ga_128BBZFV9V*czE3NzMwMjc3OTEkbzEkZzEkdDE3NzMwMjc4MzgkajEzJGwwJGgw",
    category: "GameNews",
  },
  {
    name: "株式会社スクウェア・エニックスのプレスリリース｜PR TIMES",
    url: "https://prtimes.jp/companyrdf.php?company_id=128734",
    category: "GameNews",
  },
  {
    name: "株式会社カプコン：CAPCOM WORLD JAPAN",
    url: "https://www.capcom.co.jp/rss/news.xml",
    category: "GameNews",
  },
  {
    name: "4Gamer.net",
    url: "https://www.4gamer.net/rss/index.xml",
    category: "GameNews",
  },
  {
    name: "電ファミニコゲーマー",
    url: "https://news.denfaminicogamer.jp/feed",
    category: "GameNews",
  },
  {
    name: "ゲーム総合情報サイト Gamer",
    url: " https://www.gamer.ne.jp/feed/news.rdf",
    category: "GameNews",
  },
];
